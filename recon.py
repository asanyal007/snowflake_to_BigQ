...
class Recon:
    def __init__(self, source_df, target_sampled):
        self.source_df = source_df
        self.target_sampled = target_sampled

    def count_recon(self, val):
        if self.source_df.count() == self.target_sampled.count():
            print(val + ": " + "Source and target count matching! Source:{} Targert:{}".format(self.source_df.count(), self.target_sampled.count()))
        else:
            print(val + ": " + "Source and target count not matching! Source:{} Targert:{}".format(self.source_df.count(),
                                                                                                   self.target_sampled.count()))
        return self.source_df.count() - self.target_sampled.count()

    def random_sample_check(self, guessedFraction, noOfSamples, seed):
      source_sampled = self.source_df.sample(guessedFraction, seed= seed).limit(noOfSamples)
      target_sampled = self.target_df.sample(guessedFraction, seed= seed).limit(noOfSamples)
    ...
    def dataprofile(data_all_df,data_cols):
        ...
        dprof_df.drop('column_names', inplace=True, axis=1)
        # ======================
